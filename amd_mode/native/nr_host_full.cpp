...
//
// The dispatch itself is still a passthrough (byte-identical output) until
// the HIP engine lands -- see docs/GPL_SOURCE_REQUEST.md -- but the input
// pipeline is now end-to-end real, so Phase 2b only has to swap
// DispatchPassthrough() for the HIP graph call with the SAME record.
//
...
// Placeholder for the HIP graph call. Today: passthrough (input == output).
// Phase 2b swaps this body; the signature and the record stay frozen.
...
int main(int argc, char** argv) {
    ...
    for (;;) {
        ...
        if (magic == kFrame || magic == kPrep) {
            ...
            if (!DispatchPassthrough(frame.data(), out_size(), d, out)) {
