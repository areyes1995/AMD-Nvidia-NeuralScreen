...
На машине, где нейропроход невозможен, программа всё равно открывается —
обычным окном-панелью, а не отказом запуска.

**Детект** (`gpuinfo.system_adapters`): все физические GPU перечисляются
через DXGI с PCI-id вендора (0x10DE NVIDIA, 0x1002 AMD, 0x8086 Intel;
софтверные адаптеры пропускаются). `primary_gpu` берёт NVIDIA первой —
на гибридном ноутбуке встроенная графика не должна выигрывать только
...
**Строка**: `menu_payload` несёт `system_using`
(`System using: NVIDIA RTX 4070`), `system_vendor` и `has_nvidia`;
меню рисует её отдельной приглушённой строкой под статусом
(`OverlayMenu._draw_system`). Слово вендора — это то, что гейтит фичи;
название карты говорит, какой адаптер выбран на мульти-GPU машинах.

**Деградированный цикл** (main.py): никакого блита захвата и send/recv —
прозрачный фулскрин-слой заморозил бы вид рабочего стола и съел бы мышь,
поэтому деградированный режим — это обычное рамное окно `Display`
(`plain=True`: по центру, кнопка в таскбаре, без topmost/прозрачности/
click-through/affinity захвата) с меню по ширине окна (`fit_window`).
Цикл идёт в частоте обновления монитора (`EnumDisplaySettings`,
`degraded_interval`) и перерисовывает только меню/HUD/алерты. Трей,
таскбар, хоткеи и скриншоты (один прямой граб) работают. Всё, чему нужен
воркер (`pipeline.do_restart`, `switch_monitor/window`,
`apply_spout/hdr/gpu/motion_backend`, тоггл NR, запись), — no-op с
уведомлением `Not available: worker missing` вместо падения.
